...
contract TypicalTigersMigration is
    ERC721Enumerable,
    Ownable,
    DefaultOperatorFilterer
{
    ...
    address public TigerContract = 0x8580156166e7ae7c4a050821a8F2a03bcf9d2Ee9;
    address public TigerForge = 0xac4438580BdeA883608A88922dDE0944cf32d837;
    ...
    function claimTigers(uint256[] memory theTokenIDs) public {
        ...
        for (uint256 i; i < numTokens; i++) {
            require(
                IERC721(TigerContract).ownerOf(theTokenIDs[i]) == msg.sender
            );
    ...
    function claimForgedTigers(uint256[] memory theTokenIDs) external {
        ...
        for (uint256 i; i < numTokens; i++) {
            require(
                ApeForgeInterface(TigerForge)._nftTokenForges(theTokenIDs[i]) ==
                    msg.sender,
                "Token not owned"
            );
            require(!_exists(theTokenIDs[i]), "Tiger already claimed");
